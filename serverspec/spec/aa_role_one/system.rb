...
describe "#{host_inventory['fqdn']}: System checks for aa_role_one." do
  ...
  describe 'Processes are running.' do
    ...
    describe command('/common /ptk status') do
      its(:exit_status) { is_expected.to eql(0) }
    end

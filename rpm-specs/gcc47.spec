...
#really this is version 4.7.2 but spec keeps dropping everything into 4.7.3
...
#libitm here is actually older than that provided in official el6 channels
...
%endif
Summary: GNU Compiler Collection version 4.7
Name: gcc47
Version: 4.7.2
...
%description
The gcc47 package contains the GNU Compiler Collection version 4.7.

%package c++
Summary: C++ support for GCC version 4.7
Group: Development/Languages
...
version 4.7.  It includes support for most of the current C++ specification
...
The libstdc++47 package contains a rewritten standard compliant GCC Standard
C++ Library.

%package -n libstdc++47-devel
Summary: Header files and libraries for C++ development
Group: Development/Libraries
Requires: libstdc++47 = %{version}-%{release}, %{_prefix}/%{_lib}/libstdc++.so.6
Autoreq: true
Autoprov: true
...
Group: Development/Languages
Requires: gcc47 = %{version}-%{release}
Requires: libobjc47 = %{version}-%{release}
Autoreq: true
AutoProv: true

%description objc
gcc47-objc provides Objective-C support for the GCC.
...
Summary: Objective-C++ support for GCC
Group: Development/Languages
Requires: gcc47-c++ = %{version}-%{release}, gcc47-objc = %{version}-%{release}
Autoreq: true
AutoProv: true

%description objc++
gcc47-objc++ package provides Objective-C++ support for the GCC.
...
%package gfortran
Summary: Fortran support for GCC 4.7
Group: Development/Languages
Requires: gcc47 = %{version}-%{release}
...
The gcc47-gfortran package provides support for compiling Fortran
programs with the GNU Compiler Collection.

%package -n libgfortran47
Summary: Fortran runtime for GCC 4.7
Group: System Environment/Libraries
Autoreq: true
Autoprov: true
%if %{build_libquadmath}
...
%package -n libgomp47
Summary: GCC 4.7 OpenMP v3.0 shared support library
Group: System Environment/Libraries
Requires(post): /sbin/install-info
Requires(preun): /sbin/install-info

%description -n libgomp47
This package contains GCC 4.7 shared support library which is needed
for OpenMP v3.0 support.

%package -n libmudflap47
Summary: GCC 4.7 mudflap shared support library
Group: System Environment/Libraries
...
%package -n libmudflap47-devel
Summary: GCC 4.7 mudflap support
Group: Development/Libraries
Requires: libmudflap47 = %{version}-%{release}
Requires: gcc47 = %{version}-%{release}

%description -n libmudflap47-devel
This package contains headers for building mudflap-instrumented programs.

To instrument a non-threaded program, add -fmudflap
option to GCC 4.7 and when linking add -lmudflap, for threaded programs
also add -fmudflapth and -lmudflapth.

%package -n libquadmath
Summary: GCC 4.7 __float128 shared support library
Group: System Environment/Libraries
Requires(post): /sbin/install-info
Requires(preun): /sbin/install-info
...
Summary: GCC 4.7 __float128 support
Group: Development/Libraries
Requires: libquadmath = %{version}-%{release}
Requires: gcc47 = %{version}-%{release}
...
%package gnat
Summary: Ada 95 support for GCC 4.7
Group: Development/Languages
Requires: gcc47 = %{version}-%{release}
Requires: libgnat47 = %{version}-%{release}, libgnat47-devel = %{version}-%{release}
Requires(post): /sbin/install-info
Requires(preun): /sbin/install-info
Autoreq: true
AutoProv: true
...
%description gnat
GNAT is a GNU Ada 95 front-end to GCC 4.7. This package includes development tools,
...
%description -n libgnat47
GNAT is a GNU Ada 95 front-end to GCC 4.7. This package includes shared libraries,
which are required to run programs compiled with the GNAT.

%package -n libgnat47-devel
Summary: GNU Ada 95 libraries
Group: Development/Languages
Autoreq: true
AutoProv: true

%description -n libgnat47-devel
GNAT is a GNU Ada 95 front-end to GCC 4.7. This package includes libraries,
...
Group: Development/Languages
Requires: gcc47 = %{version}-%{release}
Requires: libgo47 = %{version}-%{release}
Requires: libgo47-devel = %{version}-%{release}
Requires(post): /sbin/install-info
Requires(preun): /sbin/install-info
Autoreq: true
AutoProv: true
...
%description go
The gcc47-go package provides support for compiling Go programs
...
%description -n libgo47
This package contains Go shared library which is needed to run
Go dynamically linked programs.

%package -n libgo47-devel
...
%description -n libgo47-devel
This package includes libraries and support files for compiling
Go programs.

%package plugin-devel
Summary: Support for compiling GCC 4.7 plugins
Group: Development/Languages
Requires: gcc47 = %{version}-%{release}
...
for compiling GCC 4.7 plugins.  The GCC 4.7 plugin ABI is currently
...
mkdir -p rpm.doc/changelogs/{gcc/cp,gcc/ada,libstdc++-v3,libobjc,libmudflap,libgomp}

for i in {gcc,gcc/cp,gcc/ada,libstdc++-v3,libobjc,libmudflap,libgomp}/ChangeLog*; do
...
#in anticipation of the rename of gnatgcc to gnatgcc47 this symlink needs to resolve NOW
ln -sf gcc47 %{buildroot}%{_prefix}/bin/gnatgcc
...
#even though libgfortran has an updated A(P/B)I for gcc47 the .so versioning did not increment; idiots
...
#%find_lang gcc
#%files -f gcc.lang to package this back with gcc47
...
%post -n libstdc++47 -p /sbin/ldconfig

%postun -n libstdc++47 -p /sbin/ldconfig

%post -n libobjc47 -p /sbin/ldconfig

%postun -n libobjc47 -p /sbin/ldconfig

%post -n libgfortran47 -p /sbin/ldconfig

%postun -n libgfortran47 -p /sbin/ldconfig

%post -n libgnat47 -p /sbin/ldconfig

%postun -n libgnat47 -p /sbin/ldconfig

%post -n libgomp47 -p /sbin/ldconfig

%postun -n libgomp47 -p /sbin/ldconfig
...
%post -n libgo47 -p /sbin/ldconfig

%postun -n libgo47 -p /sbin/ldconfig
...
%{_prefix}/bin/gcov47
%ifarch ppc
%{_prefix}/bin/%{_target_platform}-gcc47
%endif
%ifarch sparc64 sparcv9
%{_prefix}/bin/sparc-%{_vendor}-%{_target_os}%{?_gnu}-gcc47
%endif
%ifarch ppc64
%{_prefix}/bin/ppc-%{_vendor}-%{_target_os}%{?_gnu}-gcc47
%endif
%dir %{_prefix}/lib/gcc
...
#libgomp for GCC 4.7.2 is identical to libgomp for 4.4.7 so lets avoid a conlict here
#%files -n libgomp47
#%defattr(-,root,root,-)
...
%changelog
* Tue Jun 3 2014 4.7.2-8.el6
- First attempt at a full GNU 4.7 toolchain side-install for EL6 (minus java, libgcc, and cpp)
